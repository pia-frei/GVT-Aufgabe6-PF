var app = ( function() {
	...
	function initModels() {
		// fillstyle
		var fs = "fill";
	    createModel("torus", fs, [ 0, 1, 0, 1 ], [1.6, 0.1, 0 ], [ 0, 0, 0 ], [
	        1, 1, 1 ]);
	    createModel("plane", "wireframe", [ 1, 1, 1, 1 ], [ 0, -.8, 0 ], [0, 0, 0 ], [ 1, 1, 1 ]);
	    createModel("sphere", fs, [ 1,0,1,1 ], [ 2,0,0 ],[ 0, 0, 0 ], [ .25, .25, .25 ]);
	    createModel("sphere", fs, [ 1, 0, 0, 1 ], [ -2,0,0 ], [ 0, 0, 0 ], [ .25, .25, .25 ]);
	    createModel("sphere", fs, [ 0, 0, 1, 1 ], [ 0,0,2 ],[ 0, 0, 0 ], [ .25, .25, .25 ]);
	    createModel("sphere", fs, [ 1, 1, 0, 1 ], [0,0,-2 ],[ 0, 0, 0 ], [ .25, .25, .25 ]);
	...
	function initEventHandler() {
		...
		window.onkeydown = function(evt) {
			...
			switch(c) {
                ...
                case('K'):
                    models[0].rotate[1] += sign * deltaRotate;
                    for (var i = 2; i < 6; i++) {
						models[i].translate[0] = Math.cos(angleRotate + (i-2) * Math.PI/2) * r;
						models[i].translate[2] = Math.sin(angleRotate + (i-2) * Math.PI/2) * r;
						models[i].rotate[1] += 2 * sign * deltaRotate;
					}
					angleRotate += sign * Math.PI / 36;
                    break;
            }
			//Scale and deform
			var deltaScale = 0.05;
